<div>
    ...
    <div wire:loading wire:target="search" class="absolute -bottom-[1px] left-0 w-full h-[2px] overflow-hidden rounded-b-xl">
        <div class="w-full h-full bg-blue-500/20">
            <div class="h-full bg-blue-500 animate-[loading_1.5s_ease-in-out_infinite]" style="width: 30%;"></div>
        </div>
    </div>
    <div wire:loading.class="opacity-50" wire:target="search">                                                        
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" wire:click="sortBy('name')" class="w-1/4 py-4 pl-6 pr-3 text-left text-sm font-semibold text-gray-900 cursor-pointer hover:text-indigo-600">Name
                    @if($sortField === 'name')
                        {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                    @else
                        ⇅   
                    ...
                    <th scope="col" wire:click="sortBy('email')" class="w-1/3 px-3 py-4 text-left text-sm font-semibold text-gray-900 cursor-pointer hover:text-indigo-600">Email
                    @if($sortField === 'email')
                        {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                     @else
                        ⇅
